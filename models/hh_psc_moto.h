...
#ifndef HH_PSC_MOTO_H
#define HH_PSC_MOTO_H
 ...
 */
extern "C" int hh_psc_moto_dynamics( double, const double*, double*, void* );

/* BeginDocumentation
Name: hh_psc_moto - Hodgkin Huxley neuron model.

Description:

 hh_psc_moto is an implementation of a spiking neuron using the Hodkin-Huxley
 formalism.
...
class hh_psc_moto : public Archiving_Node
{

public:
  hh_psc_moto();
  hh_psc_moto( const hh_psc_moto& );
  ~hh_psc_moto();
...
private:
  ...
  friend int hh_psc_moto_dynamics( double, const double*, double*, void* );

  // The next two classes need to be friend to access the State_ class/member
  friend class RecordablesMap< hh_psc_moto >;
  friend class UniversalDataLogger< hh_psc_moto >;
...
private:
  ...
  struct Buffers_
  {
    Buffers_( hh_psc_moto& );                  //!<Sets buffer pointers to 0
    Buffers_( const Buffers_&, hh_psc_moto& ); //!<Sets buffer pointers to 0

    //! Logger for all analog data
    UniversalDataLogger< hh_psc_moto > logger_;
  ...
  static RecordablesMap< hh_psc_moto > recordablesMap_;
};


inline port
hh_psc_moto::send_test_event( Node& target,
  rport receptor_type,
  synindex,
  bool )
{
...
inline port
hh_psc_moto::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
hh_psc_moto::handles_test_event( CurrentEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline port
hh_psc_moto::handles_test_event( DataLoggingRequest& dlr, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
hh_psc_moto::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d );
...
inline void
hh_psc_moto::set_status( const DictionaryDatum& d )
{
