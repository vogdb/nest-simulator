...
nest::RecordablesMap< nest::hh_psc_moto > nest::hh_psc_moto::recordablesMap_;
...
template <>
void
RecordablesMap< hh_psc_moto >::create()
{
  // use standard names whereever you can for consistency!
  insert_(
    names::V_m, &hh_psc_moto::get_y_elem_< hh_psc_moto::State_::V_M > );
  insert_( names::I_syn_ex,
    &hh_psc_moto::get_y_elem_< hh_psc_moto::State_::I_EXC > );
  insert_( names::I_syn_in,
    &hh_psc_moto::get_y_elem_< hh_psc_moto::State_::I_INH > );
  insert_(
    names::Act_m, &hh_psc_moto::get_y_elem_< hh_psc_moto::State_::HH_M > );
  insert_(
    names::Act_h, &hh_psc_moto::get_y_elem_< hh_psc_moto::State_::HH_H > );
  insert_(
    names::Inact_n, &hh_psc_moto::get_y_elem_< hh_psc_moto::State_::HH_N > );
}

extern "C" int
hh_psc_moto_dynamics( double, const double y[], double f[], void* pnode )
{
  // a shorthand
  typedef nest::hh_psc_moto::State_ S;

  // get access to node so we can almost work as in a member function
  assert( pnode );
  const nest::hh_psc_moto& node =
    *( reinterpret_cast< nest::hh_psc_moto* >( pnode ) );
...
nest::hh_psc_moto::State_::State_( const State_& s )
  : r_( s.r_ )
{
...
nest::hh_psc_moto::State_& nest::hh_psc_moto::State_::operator=(
  const State_& s )
{
...
void
nest::hh_psc_moto::Parameters_::get( DictionaryDatum& d ) const
{
...
void
nest::hh_psc_moto::Parameters_::set( const DictionaryDatum& d )
{
...
void
nest::hh_psc_moto::State_::get( DictionaryDatum& d ) const
{
...
void
nest::hh_psc_moto::State_::set( const DictionaryDatum& d )
{
...
nest::hh_psc_moto::Buffers_::Buffers_( hh_psc_moto& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::hh_psc_moto::Buffers_::Buffers_( const Buffers_&, hh_psc_moto& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::hh_psc_moto::hh_psc_moto( const hh_psc_moto& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
nest::hh_psc_moto::init_state_( const Node& proto )
{
  const hh_psc_moto& pr = downcast< hh_psc_moto >( proto );
  S_ = pr.S_;
}

void
nest::hh_psc_moto::init_buffers_()
{
  ...
  B_.sys_.function = hh_psc_moto_dynamics;
...
void
nest::hh_psc_moto::update( Time const& origin, const long from, const long to )
{
...
void
nest::hh_psc_moto::handle( SpikeEvent& e )
{
...
void
nest::hh_psc_moto::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void
nest::hh_psc_moto::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
